...
async function main() {
  const vectorStore = await openai.vectorStores.update("vs_abc123", {
    name: "Support FAQ",
  });
  console.log(vectorStore);
}
...
async function main() {
  const deletedVectorStore = await openai.vectorStores.del("vs_abc123");
...
async function main() {
  const vectorStoreFiles = await openai.vectorStores.files.list("vs_abc123");
